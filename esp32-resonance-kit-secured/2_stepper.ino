...
void stepperControl(){
  ...
  else if (state == 2 && tube_length < 85){
  ...
  //to ensure the piston does not move beyond the limit of the tube
  else if (state == 1 && tube_length <= (float)0){
    Serial.println("Invalid, tube length can't be lower than 0 cm");
    countLength();
    state = 0;
  }
  else if (state == 2 && tube_length >= (float)85){
    Serial.println("Invalid, tube length can't be greater than 85 cm");
    countLength();
    state = 0;
  }
  delay(1500);
...
void countLength(){
  tube_length = ((float)count_step)/(10*rev_step);        // every 10 revstep (2000) is +-4 cm
  //send to MQTT
  char msg_out2[4];
