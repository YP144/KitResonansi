...
void slmSampling(){
  ...
  //while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
    ...
    if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
      ...
      //send to MQTT
      char msg_out1[3];
      sprintf(msg_out1,"%.2f",Leq_dB);
      mqttClient.publish(mqtt_pub_topic1,msg_out1);
...
//loop main program 1 untuk db meter
void slmLoop(void * params) {
    for(;;){
      mic_i2s_reader_task();
      slmSampling();
    }
